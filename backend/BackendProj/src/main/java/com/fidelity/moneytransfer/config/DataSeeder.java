...
public class DataSeeder implements CommandLineRunner {
    ...
    /**
     * Backfills the new reward columns for users created before the rewards
     * feature existed, so reads never hit NULLs. Defensive: a fresh database
     * (where the columns are brand new and there are no legacy rows) is fine.
     */
    private void backfillRewardColumns() {
        try {
            jdbcTemplate.update("UPDATE users SET reward_points = 0 WHERE reward_points IS NULL");
            jdbcTemplate.update("UPDATE users SET tier = 'BRONZE' WHERE tier IS NULL");
            jdbcTemplate.update(
                    "UPDATE users SET downgrade_warning_sent = FALSE WHERE downgrade_warning_sent IS NULL");
        } catch (Exception e) {
    ...
    /**
     * Seeds the corporate CASHBACK account that funds reward cashbacks. Uses a
     * fixed id that the random account-id generator can never produce, so it is
